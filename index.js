...
import {
  ...
  onSnapshot, increment, serverTimestamp, query, orderBy, addDoc
...
function loadLocalVotes() {
  try {
    const v = localStorage.getItem('ks_votes');
    if (v) userVotes = JSON.parse(v);
  } catch {}
}
...
function loadLocalReactions() {
  try {
    ...
    if (r) {
      ...
      for (const id in parsed) userReactions[id] = new Set(parsed[id]);
    }
  } catch {}
}
...
function updateAdminVisibility() {
  // admin link shown if user UID matches
}
...
window.openProfile = function() {
  ...
  let adminBtn = document.getElementById('admin-open-btn');
  if (isAdmin && !adminBtn) {
    adminBtn = document.createElement('button');
    adminBtn.id = 'admin-open-btn';
    adminBtn.className = 'submit-btn';
    adminBtn.style.background = 'linear-gradient(135deg,#f59e0b,#d97706)';
    adminBtn.style.marginTop = '0';
    adminBtn.textContent = '⚙️ Admin Panel';
    adminBtn.onclick = () => { closeProfile(); openAdmin(); };
    document.querySelector('#profile-backdrop .modal').appendChild(adminBtn);
  }
...
function subscribeComments(itemId) {
  ...
  const q = query(collection(db, 'items', itemId, 'comments'), orderBy('createdAt', 'asc'));
  unsubComments = onSnapshot(q, (snap) => {
    ...
    list.innerHTML = snap.docs.map(d => {
      ...
      return `<div class="comment-item">
        ...
        ${canDelete ? `<button class="comment-delete" onclick="deleteComment('${itemId}','${d.id}')">✕</button>` : ''}
      </div>`;
    }).join('');
    commentCount = snap.docs.filter(d => currentUser && d.data().uid === currentUser.uid).length;
...
window.vote = async function(type) {
  ...
  try {
    ...
    // Animate card then auto-advance
    const card = document.getElementById('vote-card');
    card.style.transform = type === 'keep'
      ? 'scale(1.03) rotate(.8deg)' : 'scale(1.03) rotate(-.8deg)';
...
window.react = async function(emoji) {
  ...
  try {
    const ref = doc(db, 'items', item.id);
    await updateDoc(ref, { [`reactions.${key}`]: increment(already ? -1 : 1) });
    if (already) userReactions[item.id].delete(emoji);
    else userReactions[item.id].add(emoji);
    saveLocalReactions();

    // pop animation
    const btns = document.querySelectorAll('.reaction-btn');
    const emojiList = Object.keys(REACTION_IDS);
...
window.postComment = async function() {
  ...
  try {
    await addDoc(collection(db, 'items', items[currentIdx].id, 'comments'), {
      text, username, uid: currentUser?.uid || 'guest',
...
function render() {
  ...
  document.querySelectorAll('.reaction-btn').forEach((btn, i) => {
    const emoji = Object.keys(REACTION_IDS)[i];
    btn.classList.toggle('reacted', reacted.has(emoji));
  });
...
window.navigate = function(dir) { navigateInternal(dir); };
...
window.toggleAddForm = function() { document.getElementById('add-form').classList.toggle('open'); };
...
window.closeAdmin = function() { document.getElementById('admin-backdrop').style.display = 'none'; };

window.switchAdminTab = function(tab) {
  document.querySelectorAll('.admin-tab').forEach((t,i) => t.classList.toggle('active', (i===0&&tab==='items')||(i===1&&tab==='users')));
...
function initSwipe() {
  ...
  function onStart(x, y) { startX=x; startY=y; isDragging=true; card.style.transition='none'; }
  function onMove(x, y) {
...
initTheme();
// Auth observer handles init for signed-in users.
// Guest path is triggered by continueAsGuest().
